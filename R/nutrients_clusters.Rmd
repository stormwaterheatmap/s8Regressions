---
title: "kMeans clusters, Nutrients"
...
s8.data.df <- s8_data %>%
  ...
  # remove outliers
  group_by(parameter) %>%
  filter(!((log(new_result_value) - median(log(new_result_value))) > 3 * sd(log(new_result_value)))) %>% ungroup()
...
s8.data.df <- load_and_clean(paramgroups = c("Conventional", "Nutrient"))#,"Metal"))
...
df.by_site <- s8.data.df %>%
  ...
  remove_rownames() %>%
    ...
    t() %>%
    as.data.frame() %>%
   # mutate(total_N =`Total Kjeldahl Nitrogen.Total`+`Nitrite-Nitrate.Dissolved` ) %>% log() %>% 
