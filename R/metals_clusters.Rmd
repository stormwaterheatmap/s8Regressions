...
s8.data.df %>% select(c("site_name","parameter","new_result_value")) %>% 
...
s8.data.df %>% select(c("site_name","parameter","nondetect_flag","new_result_value")) %>% 
...
pairsplot = 
pairs(data.df %>% t(), col=c(1:3)[km$cluster])) 
)
