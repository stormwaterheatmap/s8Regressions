...
formula.2 <- (as.formula(logConcentration ~ logTraffic + percent_tree_cover + pm25 + impervious + population_density))
...
logTraffic + pm25 , data = df.coc, family = LOGNO)
...
formula.step4.1 <- update(step1.formula, . ~ . + random(AMC))
formula.step4.2 <- update(step1.formula, . ~ . + random(study_id))
formula.step4.3 <- update(step1.formula, . ~ . + random(season))
